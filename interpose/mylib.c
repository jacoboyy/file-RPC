...
int deserialize_pos =
    0; /* global position counter used in tree de-serialization*/
...
/**
 * @brief 	Interposed open function from libcs to open a file in the remote
 * server
...
int open(const char *pathname, int flags, ...) {
  ...
  memcpy(stub + SIZET_SIZE + 2 * INT_SIZE + MODET_SIZE, pathname,
         strlen(pathname) + 1);
...
/**
 * @brief 	Interposed write function from libc to write bytes to a file
 * descriptor
 * @return	the actual number of bytes written on success, -1 if an error
 * occurs
 */
...
/**
 * @brief 	Interposed read function from libc to read bytes from a file
 * descriptor
 * @return	the actual number of bytes read on success, -1 if an error
 * occurs
 */
...
/**
 * @brief 	Interposed lseek function from libc to reposition read/write file
 * offset
 * @return	the resulting file offset on success, -1 if an error occurs
 */
off_t lseek(int fd, off_t offset, int whence) {
  // check if local lseek
  if (fd < OFFSET) return orig_lseek(fd, offset, whence);
...
/**
 * @brief 	Interposed stat function from libc to get file status on remote
 * server
...
/**
 * @brief 	Interposed unlink function from libc to unlink a remote file on
 * server
 * @return	0 on success, -1 if an error occurs
 */
...
/**
 * @brief 	Interposed getdirentries function from libc to get directory
 * entries
...
ssize_t getdirentries(int fd, char *buf, size_t nbytes, off_t *basep) {
  if (fd < OFFSET) return orig_getdirentries(fd, buf, nbytes, basep);
...
struct dirtreenode *deserialize(void *buf) {
  ...
  tree->subdirs =
      (struct dirtreenode **)malloc(num_subdirs * sizeof(struct dirtreenode *));
