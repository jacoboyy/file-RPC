...
int serialize_pos =
    0; /* position in the current tree buffer used in tree serialization */
...
void rpc_getdirentries(int sessfd, void *stub) {
  ...
  *basep = *(off_t *)(stub + 2 * INT_SIZE + SIZET_SIZE);
  ...
  size_t reply_size =
      res < 0 ? SSIZET_SIZE + INT_SIZE : SSIZET_SIZE + res + OFFT_SIZE;
  void *reply_buf = malloc(reply_size);
  memcpy(reply_buf, &res, SSIZET_SIZE);
  if (res < 0)
    memcpy(reply_buf + SSIZET_SIZE, &errno, INT_SIZE);
...
/**
 * @brief Perform local getdirtree call and send back the serialized dirtreenode
 * to client
 * @param[in] sessfd  a TCP connection socket
 * @param[in] stub	  client package with serialized function arguments
 */
void rpc_getdirtree(int sessfd, void *stub) {
  ...
  // prepare reply
  size_t reply_size =
      tree_size == 0 ? SIZET_SIZE + INT_SIZE : SIZET_SIZE + tree_size;
  void *reply_buf = malloc(reply_size);
  ...
  if (tree_size > 0) freedirtree(tree);
}
